...
**CRITICAL:** Update `last_heartbeat` on EVERY state transition:
...
**Guard clause:** Only 3 states are claimable (watching, fix_proposed, exit_requested). This prevents race conditions. Verify `rows_affected = 1` after execution. The `session_id` value comes from the SessionStart hook, which injects `$CLAUDE_SESSION_ID` into the system prompt automatically.
