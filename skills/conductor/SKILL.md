...
When the Conductor itself hits context exhaustion or crashes, the Souffleur — the external supervisor — initiates recovery by either relaunching a completely new Conductor (claude_export provider) or compacting and resuming the existing session (Lethe provider), both entering via the `--recovery-bootstrap` flag. This protocol reconstructs full situational awareness from available sources: the Souffleur's export file (when present), a handoff document, MEMORY.md, the comms-link database, and a cross-session learnings file. It transforms the session into a fully operational Conductor, whether starting from a blank export-based relaunch or a compacted continuation.
